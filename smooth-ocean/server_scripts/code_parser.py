...
def list_all_paths(dummy_param: str) -> dict:
    ...
    Returns:
        dict: A dictionary with keys 'success' (bool), 'message' (str),
              and 'paths' (list[str]) containing the file paths if successful.
    """
    try:
        ...
        return {
            "success": True,
            "message": "Paths listed successfully.",
            "paths": path_dict,
        }
    except Exception as e:  # Catch any other unexpected errors
        return {
            "success": False,
            "message": "An unexpected error occurred while listing paths.",
            "paths": [],
        }

# --- MCP Server Setup ---
logging.info(
    "Creating MCP Server instance for Django Code..."
)  # Changed print to logging.info
app = Server("django-code-mcp-server") # , "0.1.0"

# Wrap database utility functions as ADK FunctionTools
ADK_DB_TOOLS = {
    "list_all_paths": FunctionTool(func=list_all_paths),
}
...
if __name__ == "__main__":
    logging.info(
        "Launching Django Code MCP Server via stdio..."
    )  # Changed print to logging.info
    try:
